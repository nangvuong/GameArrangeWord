...
public class HomeController implements Initializable {
    ...
    @FXML
    private ListView<User> onlinePlayersList;
    @FXML
                ...
                if (empty || player == null) {
                    ...
                    Tooltip tooltip = new Tooltip(
                            "Username: " + player.getUsername() + "\n" +
                                    "Status: "
                                    + (player.isPlaying() ? "In Game" : player.isOnline() ? "Online" : "Offline"));
                    setTooltip(tooltip);
                }
            }
        });
    ...
    public void showFinalMatchResult(MatchSummary matchSummary) {
        ...
        Platform.runLater(() -> {
            try {
                ...
                titleLabel.setStyle(
                        String.format("-fx-text-fill: %s; -fx-font-size: 20px; -fx-font-weight: bold;", color));
                ...
                Label totalScoreLabel = new Label(
                        String.format("Total Points: (%d) - (%d)", myTotalScore, opponentTotalScore));
